...
נסה שוב עם פרטים נוספים! 😊
    ...
    async def search_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            # השתמש ב-search_service
            results = await self.search_service.search_requests(search_term, limit=10)
            ...
            for result in results[:5]:  # הגבל ל-5 תוצאות
                status_emoji = {"pending": "⏳", "fulfilled": "✅", "rejected": "❌"}.get(result.status, "❓")
                response += f"{status_emoji} **#{result.id}** {result.title}\n"
                response += f"📅 {result.created_at.strftime('%d/%m/%Y')} | 📂 {result.category}\n\n"
            
            if len(results) > 5:
                response += f"... ועוד {len(results) - 5} תוצאות\n"
